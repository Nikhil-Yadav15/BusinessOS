...
export default function PurchasingPage() {
  ...
  const totalOutstanding = purchases.reduce((s, p) => s + parseFloat(p.balanceAmount || 0), 0);
  ...
  const fetchPurchases = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await apiClient.get('/api/purchasing/purchases', {
        token: session.token,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{purchases.length} purchase{purchases.length !== 1 ? 's' : ''} total</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">Paid to Suppliers</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">
            ₹{totalPaid.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
          </p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">Outstanding to Pay</p>
